...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!apiKey) {
      return new Response(JSON.stringify({ error: 'GEMINI_API_KEY not configured. Aggiungila nel file .env' }), { status: 500 });
    }
...
3. NON includere i backtick (\`\`\`mermaid o \`\`\`). Restituisci SOLO il codice raw del diagramma che inizia con "flowchart TD" o "graph TD".`
    ...
    if (!response.ok) {
      console.error(data);
      return new Response(JSON.stringify({ error: 'Failed to generate roadmap from AI' }), { status: 500 });
    }
